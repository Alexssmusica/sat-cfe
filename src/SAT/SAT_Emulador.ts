import { associarAssinaturaEmulador } from '../lib/acoes/associar_assinatura/Associar_Assinatura';
import { ativarSATEmulador } from '../lib/acoes/ativar_sat/Ativar_SAT';
import { atualizarSoftwareSATEmulador } from '../lib/acoes/atualizar_software_sat/Atualizar_Software_SAT';
import { bloquearSATEmulador } from '../lib/acoes/bloquear_sat/Bloquear_SAT';
import { cancelarUltimaVendaEmulador } from '../lib/acoes/cancela_ultima_venda/Cancela_Ultima_Venda';
import { comunicarCertificadoICPBRASILEmulador } from '../lib/acoes/comunicar_certificado_icpbrasil/Comunicar_Certificado_ICPBRASIL';
import { configurarInterfaceRedeEmulador } from '../lib/acoes/configurar_interface_rede/Configurar_Interface_Rede';
import { desbloquearSATEmulador } from '../lib/acoes/desbloquear_sat/Desbloquear_SAT';
import { enviarDadosVendaEmulador } from '../lib/acoes/enviar_dados_venda/Enviar_Dados_Venda';
import { testeFimFimEmulador } from '../lib/acoes/teste_fim_fim/Teste_Fim_Fim';
import { trocarCodigoDeAtivacaoEmulador } from '../lib/acoes/trocar_codigo_ativacao/Trocar_Codigo_Ativacao';
import { consultarNumeroSessaoEmulador } from '../lib/consulta/consultar_numero_sessao/Consultar_Numero_Sessao';
import { consultarSATEmulador } from '../lib/consulta/consultar_sat/Consultar_SAT';
import { consultarStatusOperacionalEmulador } from '../lib/consulta/consultar_status_operacional/Consultar_Status_Operacional';
import { extrairLogsEmulador } from '../lib/consulta/extrair_logs/Extrair_Logs';
import { SATLibrary } from '../utils/SAT_Library_Emulador';

// * MODELS
import { geraNumeroSessaoEmulador } from '../lib/acoes/gera_numero_sessao/Gera_Numero_Sessao';
import { versaoLibEmulador } from '../lib/consulta/versao_lib/Versao_Lib';
...
export class SATEmulador {
	...
	ativarSAT: (_numeroSessao: number, _subComando: number, _CNPJ: string, _cUF: number) => Promise<ModelAtivarSAT> = async (
		...
		try {
			...
			await ativarSATEmulador(_numeroSessao, _subComando, this._codigoAtivacao, _CNPJ, _cUF)
				.then((result) => {
	...
	comunicarCertificadoICPBRASIL: (_numeroSessao: number, _certificado: string) => Promise<ModelComunicarCertificadoICPBRASIL> = async (
		...
		try {
			...
			await comunicarCertificadoICPBRASILEmulador(_numeroSessao, this._codigoAtivacao, _certificado)
				.then((result) => {
	...
	enviarDadosVenda: (_numeroSessao: number, _baseCFe: string) => Promise<ModelEnviarDadosVenda> = async (
		...
		try {
			...
			await enviarDadosVendaEmulador(_numeroSessao, this._codigoAtivacao, _baseCFe)
				.then((result) => {
	...
	cancelarUltimaVenda: (_numeroSessao: number, _chave: string, _baseCFeCancelamento: string) => Promise<ModelCancelarUltimaVenda> =
		async (_numeroSessao: number, _chave: string, _baseCFeCancelamento: string): Promise<ModelCancelarUltimaVenda> => {
			try {
				...
				await cancelarUltimaVendaEmulador(_numeroSessao, this._codigoAtivacao, _chave, _baseCFeCancelamento)
					.then((result) => {
	...
	consultarSAT: (_numeroSessao: number) => Promise<ModelConsultarSAT> = async (_numeroSessao: number): Promise<ModelConsultarSAT> => {
		try {
			...
			await consultarSATEmulador(_numeroSessao)
				.then((result) => {
	 ...
	 */
	testeFimFim: (_numeroSessao: number, _baseCFe: string) => Promise<ModelTesteFimFim> = async (
		_numeroSessao: number,
		_baseCFe: string
		...
		try {
			...
			await testeFimFimEmulador(_numeroSessao, this._codigoAtivacao, _baseCFe)
				.then((result) => {
	...
	consultarStatusOperacional: (_numeroSessao: number) => Promise<ModelConsultarStatusOperacional> = async (
		...
		try {
			...
			await consultarStatusOperacionalEmulador(_numeroSessao, this._codigoAtivacao)
				.then((result) => {
	...
	consultarNumeroSessao: (_numeroSessao: number, _cNumeroSessao: number) => Promise<ModelConsultarNumeroSessao> = async (
		...
		try {
			...
			await consultarNumeroSessaoEmulador(_numeroSessao, this._codigoAtivacao, _cNumeroSessao)
				.then((result) => {
	...
	configurarInterfaceRede: (
		...
		try {
			...
			await configurarInterfaceRedeEmulador(_numeroSessao, this._codigoAtivacao, _dadosConfiguracao)
				.then((result) => {
	...
	associarAssinatura: (_numeroSessao: number, _CNPJvalue: string, _assinaturaCNPJs: string) => Promise<ModelAssociarAssinatura> = async (
		...
		try {
			...
			await associarAssinaturaEmulador(_numeroSessao, this._codigoAtivacao, _CNPJvalue, _assinaturaCNPJs)
				.then((result) => {
	...
	atualizarSoftwareSAT: (_numeroSessao: number) => Promise<ModelAtualizarSoftwareSAT> = async (
		...
		try {
			...
			await atualizarSoftwareSATEmulador(_numeroSessao, this._codigoAtivacao)
				.then((result) => {
	...
	extrairLogs: (_numeroSessao: number) => Promise<ModelExtrairLogs> = async (_numeroSessao: number): Promise<ModelExtrairLogs> => {
		try {
			...
			await extrairLogsEmulador(_numeroSessao, this._codigoAtivacao)
				.then((result) => {
	...
	bloquearSAT: (_numeroSessao: number) => Promise<ModelBloquearSAT> = async (_numeroSessao: number): Promise<ModelBloquearSAT> => {
		try {
			...
			await bloquearSATEmulador(_numeroSessao, this._codigoAtivacao)
				.then((result) => {
	...
	desbloquearSAT: (_numeroSessao: number) => Promise<ModelDesbloquearSAT> = async (
		...
		try {
			...
			await desbloquearSATEmulador(_numeroSessao, this._codigoAtivacao)
				.then((result) => {
	...
	trocarCodigoDeAtivacao: (
		...
		try {
			...
			await trocarCodigoDeAtivacaoEmulador(_numeroSessao, this._codigoAtivacao, _opcao, _novoCodigo, _confNovoCodigo)
				.then((result) => {
	...
	geraNumeroSessao: () => Promise<ModelGeraNumeroSessao> = async (): Promise<ModelGeraNumeroSessao> => {
		try {
			...
			await geraNumeroSessaoEmulador()
				.then((result) => {
