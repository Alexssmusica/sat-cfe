...
export const atualizarSoftwareSATEmulador: (_numeroSessao: number, _codigoAtivacao: string) => Promise<ModelAtualizarSoftwareSAT> =
	async function (_numeroSessao: number, _codigoAtivacao: string): Promise<ModelAtualizarSoftwareSAT> {
		return new Promise<ModelAtualizarSoftwareSAT>((resolve, reject) => {
			try {
				ffiLibraryEmulador.AtualizarSoftwareSAT.async(_numeroSessao, _codigoAtivacao, (error, resultAtualizarSoftwareSAT) => {
					if (error) {
						throw new Error(error);
					}
